...
- `artifacts/clonehero_charts_json/**.{chart,midi}.json` generated by `scripts/export_clonehero_drums_archives.py`.
  - `source: "chart"` documents were derived from `notes.chart`/`notes.txt` and output as `*.chart.json`.
  - `source: "midi"` documents were derived from `notes.mid` and output as `*.midi.json`.
  - Each JSON has: original chart/midi `path`, `timing` (tpq/us_per_beat), and per‑difficulty `events` with 8‑class `hit` and `time_sec`.
...
3) Dataset Builder from CH JSON (deliverable: `python -m chart_hero.train.build_dataset`)
   - Input: one or more roots of `**/*.{chart,midi}.json`.
...
- Dataset Builder
  - [ ] New module `chart_hero.train.build_dataset` reading `artifacts/clonehero_charts_json` (`*.chart.json` and `*.midi.json`).
...
- Window‑level sampling:
  - Actively mine windows with strong non‑drum onset energy but no charted drums (from the JSON events) and oversample them as hard negatives.
