...
def analyze_system_metrics(df):
    ...
    print("System metrics chart saved as system_usage.png")

    # Plot I/O metrics
    if (
        "disk_read_rate" in system_df.columns
        and "disk_write_rate" in system_df.columns
        ...
        # Disk I/O
        axes[0].plot(
            system_df.index, system_df["disk_read_rate_mb"], "b-", label="Read"
        )
        axes[0].plot(
            system_df.index, system_df["disk_write_rate_mb"], "r-", label="Write"
        )
        ...
        # Network I/O
        axes[1].plot(
            system_df.index, system_df["network_download_mb"], "g-", label="Download"
        )
        axes[1].plot(
            system_df.index, system_df["network_upload_mb"], "m-", label="Upload"
        )
        ...
        print("I/O metrics chart saved as io_usage.png")
...
def analyze_process_metrics(df):
    ...
    for pid in pids:
        ...
        cmdline = (
            pid_df["cmdline"].iloc[0]
            if not pid_df["cmdline"].isna().all()
            else "Unknown"
        )
    ...
    if len(top_processes) > 0:
        ...
        print("Process metrics chart saved as process_usage.png")

    # Print summary of top processes
    print("\nTop Processes by Average CPU Usage:")
    print(
        f"{'PID':<7} {'Avg CPU%':<10} {'Max CPU%':<10} {'Avg Memory':<15} {'Max Memory':<15} {'Command'}"
    )
    print("-" * 80)
    ...
    for proc in top_processes[:5]:  # Show top 5
        ...
        print(
            f"{proc['pid']:<7} {proc['avg_cpu']:<10.1f} {proc['max_cpu']:<10.1f} {avg_mem_human:<15} {max_mem_human:<15} {cmdline}"
        )
...
def analyze_gpu_efficiency(df):
    ...
    if (
        system_df.empty
        or "gpu_percent" not in system_df.columns
        or system_df["gpu_percent"].isna().all()
    ):
    ...
    if not process_df.empty and "cpu_percent" in process_df.columns:
        ...
        if avg_gpu < 30:
            ...
            print(
                "  Recommendation: Current parameters are reasonable, but could be optimized"
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze training resource monitoring logs"
    )
    parser.add_argument(
        "log_file", help="Path to the CSV log file generated by monitor_training.py"
    )
    parser.add_argument(
        "--no-plots",
        action="store_true",
