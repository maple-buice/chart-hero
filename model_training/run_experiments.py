...
log_file_path = Path(__file__).parent.parent / "logs" / f"experiment_runner_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_experiment(params: dict, use_wandb: bool, quick_test: bool, monitor_gpu: bool):
    ...
    experiment_tag_parts = [f"{key.replace('_', '')[:2]}{value}" for key, value in params.items()]
    ...
    try:
        process = subprocess.run(cmd, capture_output=True, text=True, check=False, cwd=CHART_HERO_BASE_DIR)
        end_time = time.time()
        duration = end_time - start_time

        if process.returncode == 0:
            logger.info(f"Experiment {experiment_tag} completed successfully in {duration:.2f} seconds.")
            logger.info(f"  STDOUT:\n{process.stdout[-500:]}") # Log last 500 chars of stdout
            if process.stderr:
                logger.info(f"  STDERR:\n{process.stderr[-500:]}")
            status = "success"
        else:
            logger.error(f"Experiment {experiment_tag} failed with return code {process.returncode} after {duration:.2f} seconds.")
        ...
        return {
            ...
            "train_log": str(expected_train_log_path) # train_transformer.py should create this
        }

    except Exception as e:
        end_time = time.time()
        duration = end_time - start_time
        logger.exception(f"Exception during experiment {experiment_tag} after {duration:.2f} seconds: {e}")
        return {
            "params": params,
            "tag": experiment_tag,
            "status": "exception",
...
def main():
    parser = argparse.ArgumentParser(description="Run hyperparameter tuning experiments for drum transcription.")
    parser.add_argument("--use-wandb", action="store_true", help="Enable WandB logging.")
    # Ensure the quick_test argument for run_experiments.py itself is correctly defined.
    parser.add_argument("--quick-test", action="store_true", help="Run only the first experiment combination for a quick test of the runner.")
    parser.add_argument("--monitor-gpu", action="store_true", help="Enable GPU monitoring (if applicable).")
    ...
    if args.quick_test:
        logger.info("Quick test mode is enabled for run_experiments.py, running only the first experiment combination.")
    ...
    for combination_idx, combo_values in enumerate(all_combinations):
        ...
        if run_only_first_combination:
            logger.info("Exiting after the first experiment due to --quick-test flag for run_experiments.py.")
            break
    ...
    for res in results_summary:
        ...
        if res['status'] != 'success':
            logger.info(f"  Error (if any): {res.get('error_message', 'See logs for details')}")
        if res.get('gpu_log'):
    ...
    summary_file_path = Path(__file__).parent.parent / "logs" / f"experiment_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    import json
