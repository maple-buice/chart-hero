...
def add_pedalboard_effects(audio_clip, sample_rate=44100, pb=None, room_size_range=(0.1, 0.9), cutoff_freq_hz_range=(800, 5000)):
    """
    ...
    if pb is None:
        ...
        cutoff_freq = _get_random_value(cutoff_freq_hz_range)
        pb = Pedalboard([Reverb(room_size=room_size), LowpassFilter(cutoff_frequency_hz=cutoff_freq)])
...
def add_white_noise(audio_clip, snr_db_range=(5, 30), random_state=None):
    ...
    if isinstance(random_state, int):
        random.seed(seed=random_state) # Note: This sets global seed, consider using np.random.Generator
    ...
    for func_name, params in aug_param_dict.items():
        if func_name in AVAILABLE_AUGMENTATIONS:
            ...
            try:
                ...
                aug_df[func_name] = aug_df[audio_col].progress_apply(lambda x: augmentation_func(x, **params))
            ...
            print(f"Warning: Unknown or disallowed augmentation function '{func_name}' skipped.")
...
def augment_pitch(audio_clip, sample_rate=44100, n_steps_range=(-3, 3), bins_per_octave=12,
    ...
    if not (isinstance(n_steps_range, (list, tuple)) and len(n_steps_range) == 2):
         print("Warning: n_steps_range should be a tuple/list of (min, max). Using n_steps=0.")
         n_steps = 0
    else:
    ...
    try:
        return pitch_shift(y=audio_clip, sr=sample_rate, n_steps=float(n_steps), # librosa expects float
...
def augment_spectrogram_time_masking(spec, num_masks=1, max_mask_percentage=0.1, mask_value=None, overwrite=False):
    """
    ...
    for _ in range(num_masks):
        ...
        if max_mask_width < 1: max_mask_width = 1 # Ensure at least 1 step can be masked
        mask_width = random.randint(1, max_mask_width + 1)

        # Determine start position
        if num_time_steps - mask_width < 0: continue # Skip if mask is wider than spectrogram
...
def augment_spectrogram_frequency_masking(spec, num_masks=1, max_mask_percentage=0.15, mask_value=None, overwrite=False):
    """
    ...
    for _ in range(num_masks):
        ...
        if max_mask_height < 1: max_mask_height = 1 # Ensure at least 1 bin can be masked
        mask_height = random.randint(1, max_mask_height + 1)

        # Determine start position
        if num_freq_bins - mask_height < 0: continue # Skip if mask is taller than spectrogram
...
def get_span_indices(dim, min_span=5, max_span=None, span_variation=0):
    ...
    rand_min_span = min_span + random.randint(low=-span_variation, high=span_variation + 1)
    if rand_min_span <= 0: rand_min_span = 1
    
    rand_max_span = max_span
    if max_span is not None:
        rand_max_span = max_span + random.randint(low=-span_variation, high=span_variation + 1)
        if rand_max_span < rand_min_span: rand_max_span = rand_min_span # Ensure max >= min
        if rand_max_span <= 0: rand_max_span = 1
    ...
    # Determine start position
    if dim - span_len < 0: # Should not happen with the min(span_len, dim) above, but safe check
        start_ind = 0
        span_len = dim
    else:
...
def compare_waveforms(df, i, signal_cols, signal_labs=None, sample_rate=44100, max_pts=None, alpha=0.5, fontsizes=[24, 18, 20], figsize=(16, 12), leg_loc='best', title=''):
    ...
    for col, lab, alp in list(zip(signal_cols, signal_labs, alpha)):
        ...
        librosa.display.waveplot(audio_data, sr=sample_rate, max_points=max_pts, label=lab, alpha=alp) 
...
def augment_spectrogram_spans(spec, spans=3, span_ranges=[[1,4], [1,6]], span_variation=1,
    ...
    if not ind_lists:
        ...
        for i in range(0, spans):
            ...
            time_inds = get_span_indices(dims[1], min_span=span_ranges[0][0], max_span=span_ranges[0][1],
                                 span_variation=span_variation)
            freq_inds = get_span_indices(dims[0], min_span=span_ranges[1][0], max_span=span_ranges[1][1],
                                 span_variation=span_variation)
            ind_lists.append([time_inds, freq_inds]) # Store as [time_indices, freq_indices]
