...
def test_data_loading():
    ...
    try:
        ...
        data_prep = data_preparation(
            directory_path="datasets/e-gmd-v1.0.0", dataset="egmd", sample_ratio=0.001
        )
        data_prep.create_audio_set(dir_path="datasets/processed", num_batches=1)
        ...
        for i, batch in enumerate(train_loader):
            ...
            if len(set(spec_shapes)) == 1:
                print(
                    f"    ✅ All spectrograms have consistent shape: {spec_shapes[0]}"
                )
            else:
