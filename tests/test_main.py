...
def test_main_script(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, 'argv', [
        ...
        '-p', str(dummy_audio_path),
        '-km', 'speed',
        '--model-path', dummy_model_path,
        '-o', str(tmp_path)
    ])
    
    with patch('chart_hero.main.identify_song') as mock_identify_song:
        mock_identify_song.return_value = {'title': 'test_song', 'artist': 'test_artist', 'musicbrainz': [{'id': '123'}]}
        with patch('chart_hero.main.get_data_from_acousticbrainz') as mock_get_data_from_acousticbrainz:
            mock_get_data_from_acousticbrainz.return_value = {}
            with patch('chart_hero.main.drum_extraction') as mock_drum_extraction:
                mock_drum_extraction.return_value = (np.random.randn(sample_rate * 5), sample_rate)
