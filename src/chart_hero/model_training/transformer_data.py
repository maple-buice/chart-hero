...
from .transformer_config import BaseConfig, get_drum_hits
...
class NpyDrumDataset(Dataset):
    ...
    def _load_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for spec_file, label_file in self.data_files:
            try:
                ...
                logger.error(f"Error loading data from {spec_file} or {label_file}: {e}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            'track_id': torch.tensor(idx, dtype=torch.long), # Using index as track_id for now
            'start_time': torch.tensor(0.0, dtype=torch.float32), # No timing info in .npy files
...
def create_data_loaders(config: BaseConfig, 
    ...
    # Create datasets
    train_dataset = NpyDrumDataset(data_files['train'], config, mode='train', augment=True)
    val_dataset = NpyDrumDataset(data_files['val'], config, mode='val', augment=False)
    test_dataset = NpyDrumDataset(data_files['test'], config, mode='test', augment=False)
