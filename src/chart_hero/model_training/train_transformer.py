...
class DrumTranscriptionModule(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        device_type = (
            "mps"
            if torch.backends.mps.is_available() and config.device == "mps"
            else "cpu"
        )
...
def setup_logger(
    config,
    project_name: str = "chart-hero-transformer",
    use_wandb: bool = True,
...
def train_model(
    ...
    if use_wandb_logging:
        ...
        wandb_logger_instance = setup_logger(
            config,
    ...
    trainer = pl.Trainer(
        ...
        limit_val_batches=(
            2 if hasattr(config, "is_quick_test") and config.is_quick_test else None
        ),
    )
...
def setup_arg_parser():
    ...
    parser.add_argument(
        "--monitor-gpu",
        action="store_true",
        help="Flag to indicate GPU monitoring is active (for internal script awareness).",
    )
    parser.add_argument(
        "--batch-size", type=int, help="Override batch size from config."
    )
    parser.add_argument(
        "--hidden-size", type=int, help="Override hidden size from config."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-workers",
        type=int,
        help="Override number of data loader workers from config.",
    )
    parser.add_argument(
        "--accumulate-grad-batches",
        type=int,
        help="Override gradient accumulation batches from config.",
    )
...
def configure_paths(config, args):
    if args.data_dir:
        ...
        config.data_dir = str(
            Path(__file__).resolve().parent.parent / "datasets" / "processed"
        )
        logger.warning(
            f"data_dir not found in config or CLI, using default: {config.data_dir}"
        )
    ...
    if args.audio_dir:
        ...
        config.audio_dir = str(
            Path(__file__).resolve().parent.parent / "datasets" / "e-gmd-v1.0.0"
        )
        logger.warning(
            f"audio_dir not found in config or CLI, using default: {config.audio_dir}"
        )
    ...
    if not hasattr(config, "model_dir") or not config.model_dir:
        ...
        config.model_dir = str(
            Path(config.model_dir).resolve() / args.experiment_tag
        )  # Use experiment_tag_to_use
    ...
    if not hasattr(config, "log_dir") or not config.log_dir:
        ...
        logger.info(
            f"Log directory not specified in config, defaulting to: {config.log_dir}"
        )
    else:
...
def apply_cli_overrides(config, args):
    ...
    if args.num_workers is not None:
        ...
        logger.info(
            f"Overriding number of workers with CLI argument: {args.num_workers}"
        )

    if args.accumulate_grad_batches is not None:
        config.accumulate_grad_batches = args.accumulate_grad_batches
        logger.info(
            f"Overriding accumulate_grad_batches with CLI argument: {config.accumulate_grad_batches}"
        )
    ...
    if args.quick_test:
        ...
        config.batch_size = getattr(
            config,
            "quick_test_batch_size",
            min(
                (
                    config.batch_size
        ...
        logger.info(
            f"Quick test mode enabled via CLI. Epochs: {config.num_epochs}, Batch Size: {config.batch_size}"
        )
...
def configure_run(args):
    ...
    args.experiment_tag = experiment_tag_to_use

    # Load configuration first
    config_profile_name = args.config
    if config_profile_name.lower() == "auto":
        logger.info(
            "Configuration profile set to 'auto'. Auto-detecting configuration..."
        )
    ...
    if effective_use_wandb:
        ...
        if wandb.run is None:
            ...
            wandb_log_dir = getattr(
                config, "log_dir", "./wandb_logs"
            )  # Default log_dir if not in config
            Path(wandb_log_dir).mkdir(
    ...
    setup_experiment_logging(
        config.log_dir, experiment_tag_to_use
    )  # Use experiment_tag_to_use

    if not hasattr(config, "data_dir") or not Path(config.data_dir).exists():
        logger.error(
            f"Data directory does not exist: {getattr(config, 'data_dir', 'Not Set')}"
        )
        sys.exit(1)
    validate_config(config)

    logger.info(
        f"Final configuration for run {args.experiment_tag}: {config.__dict__ if hasattr(config, '__dict__') else config}"
    )
    return config, effective_use_wandb, experiment_tag_to_use


def main():
    parser = setup_arg_parser()
    args = parser.parse_args()
    config, effective_use_wandb, experiment_tag_to_use = configure_run(args)
    ...
    trained_model, trainer_instance = (
        None,
        None,
    )  # Initialize to ensure they are defined
    try:
        logger.info("Starting model training...")
            ...
            if not (hasattr(config, "is_quick_test") and config.is_quick_test):
                if test_loader:
                    ...
                    trainer_instance.test(
                        model=trained_model, dataloaders=test_loader
        ...
        if args.use_wandb and wandb.run:
            ...
            exit_code = (
                wandb.run._exit_code
                if hasattr(wandb.run, "_exit_code") and wandb.run._exit_code is not None
                else 0
            )
