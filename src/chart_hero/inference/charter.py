...
class Charter:
    ...
    def predict(self, segments: Sequence[Segment | torch.Tensor]) -> pd.DataFrame:
        ...
        if offset_frames > 0:
            trimmed_segments: list[Segment] = []
            for seg in norm_segments:
                start = int(seg["start_frame"])
                end = int(seg["end_frame"])
                spec: torch.Tensor = seg["spec"]
                trim = max(0, min(offset_frames - start, spec.shape[1]))
                if trim > 0:
                    spec = spec[:, trim:]
                    start += trim
                    end -= trim
                ...
                total = int(seg.get("total_frames", end) - offset_frames)
                if spec.shape[1] > 0 and end > start:
                    seg.update(
                        {
                            "spec": spec,
                            "start_frame": start,
            ...
            norm_segments = trimmed_segments
